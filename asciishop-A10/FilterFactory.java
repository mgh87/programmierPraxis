...
public class FilterFactory implements Factory {
	...
	private BlockGenerator getBorderMode(String s, int size) throws FactoryException {
		if("x".equals(s))
			...
			throw new FactoryException("FilterFactory: unknown border mode: '" 
				+ s + "'");
	}
	...
	public Operation create(Scanner scanner) throws FactoryException {
		...
		while(sc.hasNext()) {
			if(sc.hasNextInt()) {
				size = sc.nextInt();
				if ( (size + 1) % 2 != 0)
					throw new FactoryException("FilterFactory: size needs to be odd");
			} else {
		...
		if("median".equals(command))
			...
			throw new FactoryException("FilterFactory: unknown filter mode - '" 
				+ command + "'");
	}
