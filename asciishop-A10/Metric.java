...
public interface Metric<T> {
     ...
     * as a measure of similarity. The implementor must ensure that following
     * properties of a metric are fulfilled:
     * 1.) <code>distance(x,y) >= 0</code>. It is also required that
     *     <code>distance(x,y) == 0</code>,
     * if <code>x.equals(y)</code>. Note that <code>distance(x,y) ==
     * 0</code>
     * might hold, even if <code>!x.equals(y)</code> (semimetric).
